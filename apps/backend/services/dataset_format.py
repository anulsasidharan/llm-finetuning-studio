...
Ported from `training_engine/datasets/formatter.py` rather than imported,
since `apps/backend` and `training_engine` are separate standalone
processes/environments per CLAUDE.md's architecture decision. Keep the two
copies in sync if either evolves.
